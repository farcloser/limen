...
| `skills/` *(planned)* | AI agent skills — packaged instructions that let a coding agent apply our practices directly, invoking `limen` where machine verification is needed. How an agent gets to *commit and push* at all — its own account, an enclave-bound key, the sandbox — is [`book/agents.md`](./book/agents.md). |
